...
elseif($_SERVER['REQUEST_METHOD'] === "POST"){// if other condition isnt met and method = post
    $usr=login_builder(dbconnect_select(),$_POST);
    echo $usr["password"];
    if ($usr&& password_verify($_POST["password"],$usr["password"])){
        $_SESSION["builder"] = true;
        $_SESSION["builderid"] = $usr["builderid"];
        $_SESSION["usermessage"] = "login success";
        builderauditor(dbconnect_insert(),$_SESSION["builderid"],"log","user has succesfully logged in");
