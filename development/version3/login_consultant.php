...
elseif($_SERVER['REQUEST_METHOD'] === "POST"){// if other condition isnt met and method = post
    $usr=login_consultant(dbconnect_select(),$_POST);
    echo $usr["password"];
    if ($usr&& password_verify($_POST["password"],$usr["password"])){
        $_SESSION["consultant"] = true;
        $_SESSION["consultantid"] = $usr["consultantid"];
        $_SESSION["usermessage"] = "login success";
        consultantauditor(dbconnect_insert(),$_SESSION["consultantid"],"log","user has succesfully logged in");
